...
def propose_eagle3_tokens(
    ...
    fused = drafter.feature_fuser(fuse_hidden_states(outputs.hidden_states, selected_layers)[:, -1, :])
...
def run_eagle3_speculative_decode(
    ...
    while len(generated) < max_new_tokens:
        ...
        draft_ids = propose_eagle3_tokens(target_model, drafter, prefix, selected_layers, requested)
...
def main() -> None:
    ...
    for prompt in prompts:
        ...
        if baseline_tokens and generated_tokens != baseline_tokens:
            raise AssertionError("EAGLE-3 greedy output diverged from baseline")
