...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark n-gram speculative decoding with vLLM.")
    ...
    parser.add_argument("--vllm-ngram-method", choices=("ngram", "ngram_gpu"), default="ngram")
...
def build_baseline_summary(
    ...
    if args.serial_prompts:
        ...
        return {
            "method": "ngram_vllm_baseline_serial",
            "model": args.model_path,
...
def main() -> None:
    ...
    baseline_summary_path = (
        ...
        else Path(f"runs/ngram_vllm_len{args.draft_len}.baseline.json")
    )
    ...
    output_path = (
        Path(args.output)
        if args.output
        else Path(f"runs/ngram_vllm_len{args.draft_len}.summary.json")
    )
    output_path.parent.mkdir(parents=True, exist_ok=True)
    summary = {
        "method": "ngram_vllm",
        "vllm_ngram_method": args.vllm_ngram_method,
        "model": args.model_path,
        "num_prompts": len(prompt_inputs),
        "draft_len": args.draft_len,
        "prompt_lookup_min": args.prompt_lookup_min,
